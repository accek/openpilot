...
def get_RadarState_from_vision(lead_msg: capnp._DynamicStructReader, v_ego: float, model_v_ego: float, a_ego: float):
...
def get_lead(v_ego: float, a_ego: float, ready: bool, tracks: dict[int, Track], lead_msg: capnp._DynamicStructReader,
  ...
  if track is not None:
    ...
    lead_dict = get_RadarState_from_vision(lead_msg, v_ego, model_v_ego, a_ego)
...
class RadarD:
  ...
  def update(self, sm: messaging.SubMaster, rr: car.RadarData):
    ...
    if len(leads_v3) > 1:
      self.radar_state.leadOne = get_lead(self.v_ego, self.a_ego, self.ready, self.tracks, leads_v3[0], model_v_ego, self.CP, self.CP_SP, low_speed_override=True)
      self.radar_state.leadTwo = get_lead(self.v_ego, self.a_ego, self.ready, self.tracks, leads_v3[1], model_v_ego, self.CP, self.CP_SP, low_speed_override=False)
