...
class LatControl(ABC):
  ...
  def _check_saturated(self, saturated, CS, steer_limited_by_safety, curvature_limited):
    return self.__check_saturation(1, saturated, CS, steer_limited_by_safety, curvature_limited)

  def __check_saturation(self, key, saturation, CS, steer_limited_by_safety, curvature_limited):
    if (saturation or curvature_limited) and CS.vEgo > self.sat_check_min_speed and not steer_limited_by_safety and not CS.steeringPressed:
      self.sat_count[key] += self.sat_count_rate
    elif not steer_limited_by_safety:
      self.sat_count[key] -= self.sat_count_rate
