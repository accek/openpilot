...
class LatControlAngle(LatControl):
  ...
  def update(self, active, CS, VM, params, steer_limited_by_controls, desired_curvature, calibrated_pose, curvature_limited, model_data=None):
    ...
    angle_log.saturated = self._check_saturated(angle_control_saturated, CS, False, curvature_limited)
    angle_log.steeringAngleDeg = float(CS.steeringAngleDeg)
    angle_log.steeringAngleDesiredDeg = angle_steers_des
    angle_log_ac.saturating = self._check_saturating(angle_control_saturating, CS)
    return 0, float(angle_steers_des), angle_log, angle_log_ac
